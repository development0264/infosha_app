...
class SocialMediaFacebookView extends StatefulWidget {
  List<GetSocial> nickName;

  SocialMediaFacebookView({Key? key, required this.nickName}) : super(key: key);
...
class _AddressViewState extends State<SocialMediaFacebookView> {
  ...
  void initState() {
    ...
    if (widget.nickName.isNotEmpty) {
      ...
      widget.nickName
          .removeWhere((address) => address.social!.facebookId == null);
      if (widget.nickName.isNotEmpty) {
        String formattedAddress = [widget.nickName.first.social!.facebookId]
  ...
  @override
  void didUpdateWidget(covariant SocialMediaFacebookView oldWidget) {
    if (widget.nickName.isNotEmpty) {
      setState(() {
        isLoaidng = true;
      });
            ...
            : PopupMenuButton(
                ...
                child: Row(
                  ...
                  children: [
                    InkWell(
                      onTap: () {
                        openFacebookProfile(selectedName);
                      },
                      child: Container(
                        constraints: BoxConstraints(
                ...
                itemBuilder: (context) {
                  return widget.nickName.map((e) {
                    String formattedAddress = [e.social!.facebookId]
                        .where((part) => part != null)
                        .join(', ');
