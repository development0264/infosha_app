...
class EditBioUnregister extends StatefulWidget {
  String id;
  UnregisteredUserModel viewProfileModel;
  EditBioUnregister(
      {Key? key, required this.id, required this.viewProfileModel})
      : super(key: key);
...
class _EditBioScreenState extends State<EditBioUnregister> {
  ...
  void initState() {
    ...
    setState(() {
      name.text = widget.viewProfileModel.data!.name!;

      profession.text = widget.viewProfileModel.data!.profession ?? "";

      if (widget.viewProfileModel.data!.gender != null &&
          widget.viewProfileModel.data!.gender != "") {
        gender =
            toBeginningOfSentenceCase(widget.viewProfileModel.data!.gender)!;
      }

      if (widget.viewProfileModel.data!.dob != null &&
          widget.viewProfileModel.data!.dob != "") {
        dateofbirthController.text = DateFormat('yyyy-MM-dd').format(
            DateFormat("yyyy-MM-dd").parse(widget.viewProfileModel.data!.dob!));
      }
              ...
              return CustomButton(
                () async {
                  if (_form.currentState!.validate()) {
                    ...
                    payload.addAll({"number": widget.id});
                    // payload.addIf(
                    //     name.text.trim().isNotEmpty, "name", name.text.trim());
                    payload.addIf(gender != "Gender", "gender", gender);
                    payload.addIf(selectedProfession != "Profession",
                        "profession", selectedProfession);
                    // payload.addIf(profession.text.trim().isNotEmpty,
                    ...
                    if (pickedFile != null) {
                      payload.addIf(pickedFile != null, "photo",
                          base64.encode(await pickedFile!.readAsBytes()));
                    }
                ...
                text: provider.isEditUnregisterUser
                    ? "loading"
                    : "Save Changes".tr,
                color: primaryColor,
  ...
  Container profileicon(BuildContext context) {
    return Container(
      width: 100,
      height: 100,
